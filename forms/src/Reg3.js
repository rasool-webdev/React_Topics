  ...
  return (
		<div
			...
			<form className=' border border-dark h-20'>
				...
				<input ref={inpref} type='email' name='' id='email' className='border border-dark' />{" "}

				<br />
				<label htmlFor='password'>Password</label>
				<input type='password' ref={passref} className='border border-dark' /> 
                <br />
				{erros.password && <p className='text-danger'>{erros.password}</p>}
				<button className='border border-black-200 bg-black p-2 text-white' onClick={handleClick}>clic</button>
			</form>
		</div>
	)
}
